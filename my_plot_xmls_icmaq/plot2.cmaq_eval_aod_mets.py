...
### PASSED AGRUEMENTS
if len(sys.argv) < 8:
    print("you must set 7 arguments as area title_area [day1|day2] start_date end_date run_cycle aod_qc_flag case_id")
    sys.exit()
else:
    area = sys.argv[1]
...
title_var =eval_case_id+"_aod_"+stat_var[0:2]
...
ymin="-0.5"
ybuf="0.01"
models = [ "cmaqaodpara8"+aod_qc_flag.lower(), "cmaqaodprod"+aod_qc_flag.lower() ]
lend_mdl = [ "PROD" ]
lend_mdl = [ "PROD", "V150a" ]
lend_obs = [ "OBS" ]
...
with open(plot_xml_file, 'a') as xml:
    ...
    date = sdate
    while date <= edate:
        verifout=MET_verif_out_dir+date.strftime(title_date_format)+"/cmaqaod_prod_"+aod_qc_flag.lower()+"_"+date.strftime(title_date_format)+".stat.tar"
    ...
    while date <= edate:
        ...
        date = date + hour_inc
    ### date = sdate
    ### only with observed values
    ### while date <= edate:
    ###     obs_dir=GOES_aod_obs_dir+date.strftime(title_date_format)
    ###     if os.path.exists(obs_dir):
    ###         obsfile=os.path.join(obs_dir, obs_fhead+date.strftime(title_date_format)+"_"+date.strftime(title_hour_format)+"_"+aod_qc_flag.lower()+obs_ftail)
    ...
    xml.write("        <tmpl>\n")
    xml.write("            <data_file>"+label_area+"_"+plot_var.upper()+"_"+verf_day_id.upper()+"_"+verf_cycle_id.upper()+"_"+aod_qc_flag.upper()+"_"+sdate.strftime(database_date_format)+".data</data_file>\n")
    xml.write("            <plot_file>"+label_area+"_"+plot_var.upper()+"_"+verf_day_id.upper()+"_"+verf_cycle_id.upper()+"_"+aod_qc_flag.upper()+"_"+sdate.strftime(database_date_format)+".png</plot_file>\n")
    xml.write("            <r_file>plot_"+label_area+"_"+plot_var.upper()+"_"+verf_day_id.upper()+"_"+verf_cycle_id.upper()+"_"+aod_qc_flag.upper()+".R</r_file>\n")
    xml.write("            <title>MET_"+title_var.upper()+"_"+verf_day_id.upper()+"_"+verf_cycle_id.upper()+"_"+aod_qc_flag.upper()+"_"+sdate.strftime(title_time_format)+"_"+edate.strftime(title_time_format)+" - "+label_area+"</title>\n")
    xml.write("            <x_label>TIME</x_label>\n")
    xml.write("            <y1_label>ME</y1_label>\n")
    xml.write("            <y2_label/>\n")
    xml.write("            <caption/>\n")
    xml.write("            <job_title>plot_"+label_area+"_"+plot_var.upper()+"_"+verf_day_id.upper()+"_"+verf_cycle_id.upper()+"_"+aod_qc_flag.upper()+"_"+database_date+"</job_title>\n")
    ...
    xml.write("        <title_size>1.8</title_size>\n")
    ...
    xml.write("        <xtlab_freq>3</xtlab_freq>\n")
    xml.write("        <xtlab_size>1.2</xtlab_size>\n")
    xml.write("        <xlab_weight>1</xlab_weight>\n")
    xml.write("        <xlab_size>2</xlab_size>\n")
    xml.write("        <xlab_offset>4</xlab_offset>\n")
    ...
    xml.write("        <lines>\n")
    xml.write("            <line color=\"#ff00ff\" line_pos=\"0\" lty=\"2\" lwd=\"2\" type=\"horiz_line\"/>\n")
    xml.write("        </lines>\n")
    ...
    xml.write("        <order_series>c(2,1)</order_series>\n")
    xml.write("        <plot_cmd>abline(h=0, col=\"black\", lwd=2, lty=2)</plot_cmd>\n")
    xml.write("        <legend>c(\"PARA8\",\"PROD\")</legend>\n")
