...
elif stat_var == "pm25dl":
    plot_var = eval_id+"_pm25_diurnal"
    figfile = eval_id+"_"+obs_id+"_pm25_diurnal"
else:
    plot_var = eval_id+"_pm25_"+stat_var.lower()
    figfile = eval_id+"_"+obs_id+"_pm25_"+stat_var.lower()

y_label="PM25 CONC (ug/m^3)"
### PASSED AGRUEMENTS
if len(sys.argv) < 6:
    print("you must set 7 arguments as area title_area [day1|day2] start_date end_date run_cycle")
    sys.exit()
else:
    area = sys.argv[1]
...
with open(plot_xml_file, 'a') as xml:
    ...
    xml.write("                <set name=\"fcst_valid_beg_1\">\n")
    date = sdate
    while date <= edate:
    ...
    xml.write("        </plot_fix>\n")
    xml.write("        <plot_cond/>\n")
    xml.write("        <indep equalize=\""+equalize_index+"\" name=\"fcst_lead\">\n")
    vhour = vhour_beg
    ...
    xml.write("        <legend_ncol>5</legend_ncol>\n")
    ...
    xml.write("        <plot_ci>c(\"none\",\"none\",\"none\",\"none\",\"none\",\"none\",\"none\",\"none\")</plot_ci>\n")
    xml.write("        <show_signif>c(FALSE,FALSE,FALSE,FALSE,FALSE,FALSE,FALSE,FALSE)</show_signif>\n")
    xml.write("        <plot_disp>c(TRUE,TRUE,TRUE,TRUE,FALSE,FALSE,FALSE,TRUE)</plot_disp>\n")
    xml.write("        <colors>c(\"#ff0000FF\",\"#0000ffFF\",\"#ff0000FF\",\"#0000ffFF\",\"#ff0000FF\",\"#0000ffFF\",\"#ff0000FF\",\"#000000FF\")</colors>\n")
    xml.write("        <pch>c(20,20,20,20,20,20,20,20)</pch>\n")
    xml.write("        <type>c(\"l\",\"l\",\"l\",\"l\",\"p\",\"p\",\"p\",\"p\")</type>\n")
    xml.write("        <lty>c(2,2,1,1,2,2,1,1)</lty>\n")
    xml.write("        <lwd>c(3,3,3,3,3,3,3,3)</lwd>\n")
    xml.write("        <con_series>c(0,0,0,0,0,0,0,0)</con_series>\n")
    xml.write("        <order_series>c(5,3,4,2,6,7,8,1)</order_series>\n")
    xml.write("        <plot_cmd/>\n")
    xml.write("        <legend>c(\"PARA6A_BC\",\"PROD_BC\",\"PARA6A\",\"PROD\",\"\",\"\",\"\",\"OBS\")</legend>\n")
