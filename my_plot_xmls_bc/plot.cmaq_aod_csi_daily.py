...
if stat_var == "aod":
    plot_var = "cmaq_time_series"
else:
    plot_var = "cmaq_aod_"+stat_var.lower()

### PASSED AGRUEMENTS
if len(sys.argv) < 8:
    print("you must set 5 arguments as area title_area [day1|day2] start_date end_date run_cycle csi_thresh aod_qc_flag")
    sys.exit()
else:
    area = sys.argv[1]
...
with open(plot_xml_file, 'a') as xml:
    ...
    xml.write("                </set>\n")
    xml.write("            </field>\n")
    xml.write("            <field equalize=\"true\" name=\"obs_thresh\">\n")
    ...
    xml.write("        </plot_fix>\n")
    xml.write("        <plot_cond/>\n")
    xml.write("        <indep equalize=\"true\" name=\"fcst_init_beg\">\n")
    date = sdate
    while date <= edate:
    ...
    xml.write("        <tmpl>\n")
    xml.write("            <data_file>"+label_area+"_"+plot_var.upper()+"_"+verf_day_id.upper()+"_"+verf_cycle_id.upper()+"_"+aod_qc_flag.upper()+"_"+sdate.strftime(database_date_format)+"_"+csi_thresh+".data</data_file>\n")
    xml.write("            <plot_file>"+label_area+"_"+plot_var.upper()+"_"+verf_day_id.upper()+"_"+verf_cycle_id.upper()+"_"+aod_qc_flag.upper()+"_"+sdate.strftime(database_date_format)+"_"+csi_thresh+".png</plot_file>\n")
    ...
    xml.write("            <title>MET_"+plot_var.upper()+"_Verification_"+verf_day_id.upper()+"_"+verf_cycle_id.upper()+"_"+aod_qc_flag.upper()+"_"+database_date+" - "+label_area+"</title>\n")
    xml.write("            <x_label>Day of the month</x_label>\n")
    xml.write("            <y1_label>"+stat_var.upper()+"</y1_label>\n")
    xml.write("            <y2_label/>\n")
    xml.write("            <caption/>\n")
    xml.write("            <job_title>plot_"+label_area+"_"+plot_var.upper()+"_"+verf_day_id.upper()+"_"+verf_cycle_id.upper()+"_"+aod_qc_flag.upper()+"_"+database_date+"_"+csi_thresh+"</job_title>\n")
