...
### PASSED AGRUEMENTS
if len(sys.argv) < 6:
    print("you must set 5 arguments as area title_area [day1|day2] start_date end_date run_cycle")
    sys.exit()
else:
    area = sys.argv[1]
...
with open(plot_xml_file, 'a') as xml:
    ...
    xml.write("    </folders>\n")
    xml.write("    <plot>\n")
    xml.write("        <template>bar_plot.R_tmpl</template>\n")
    xml.write("        <dep>\n")
    xml.write("            <dep1>\n")
    ...
    xml.write("        </indep>\n")
    xml.write("        <calc_stat>\n")
    xml.write("            <calc_ctc>true</calc_ctc>\n")
    xml.write("        </calc_stat>\n")
