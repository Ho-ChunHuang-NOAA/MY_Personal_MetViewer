...
tmp_data_dir="/lfs/h2/emc/stmp/"+os.environ['USER']+"/working/check_fcst_lead_plusfvs_"+database_date
...
ybuf="0.01"
ybuf="0.04"
models = [ "hysplit", "hysplitdustprod" ]
lend_mdl = [ "PROD" ]
lend_obs = [ "OBS" ]
regs = [ "FULL", "G236", "G245", "G246" ]
csi_thresh = [ "1.0", "2.0", "5.0", "10.0", "15.0", "20.0" ]
...
with open(plot_xml_file, 'a') as xml:
    ...
    xml.write("        <database>mv_grid2grid_fvs_verf_hysplitdust_"+database_date+",mv_grid2grid_met_verf_hysplitdust_"+database_date+"</database>\n")
    ...
    if area == "FULL":
        xml.write("                    <val>CS</val>\n")
        xml.write("                    <val>FULL</val>\n")
    else:
        xml.write("                    <val>"+area+"</val>\n")
        xml.write("                    <val>"+area+"</val>\n")
    xml.write("                </set>\n")
    ...
    xml.write("            <val label=\"&lt;1.0\" plot_val=\"\">&lt;1.0</val>\n")
    ...
    xml.write("        <tmpl>\n")
    xml.write("            <data_file>"+label_area+"_"+plot_var.upper()+"_"+verf_day_id.upper()+"_"+verf_cycle_id.upper()+"_"+sdate.strftime(database_date_format)+".data</data_file>\n")
    xml.write("            <plot_file>"+label_area+"_"+plot_var.upper()+"_"+verf_day_id.upper()+"_"+verf_cycle_id.upper()+"_"+sdate.strftime(database_date_format)+".png</plot_file>\n")
    xml.write("            <r_file>plot_"+label_area+"_"+plot_var.upper()+"_"+verf_day_id.upper()+"_"+verf_cycle_id.upper()+".R</r_file>\n")
    xml.write("            <title>"+plot_var.upper()+"_Verification_"+verf_day_id.upper()+"_"+verf_cycle_id.upper()+"_"+database_date+" - "+label_area+"</title>\n")
    xml.write("            <x_label>OBS Threshold - Column Mean 0-5000m ug/m3</x_label>\n")
    ...
    xml.write("        <plot_cmd/>\n")
    xml.write("        <legend>c(\"VSDB\",\"MET\")</legend>\n")
